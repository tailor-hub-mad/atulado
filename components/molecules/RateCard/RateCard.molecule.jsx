...
export default function RateCard({ icon, description, name, hours, rates, ...props }) {
  const displayHours = () => {
          ...
          <div className="green-card-wrapper">
            ...
            <div className="info">
              {Object.keys(info).map(key => {
                return (
                  <div key={uniqueId()} className={key === "Potencia" ? 'potencia-item info-item' : 'info-item'}>
                    <SCTextXSLight>
                      {key}
                    </SCTextXSLight>
                    <SCTextXSLight>
  ...
  return (
    <SCRateCard hours={hours?.length || 1}>
      ...
      <SCTextSLight className="rate-description">
        {description}
      </SCTextSLight>
      {hours &&
        <div className="hours">
